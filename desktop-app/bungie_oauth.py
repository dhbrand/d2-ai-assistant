...
class OAuthManager:
    ...
    def get_headers(self):
        """Get the necessary headers for API requests, refreshing token if needed."""
        if not self.refresh_if_needed():
            logger.error("Failed to refresh token or no token available. Cannot get headers.")
            # Optionally, trigger re-authentication here or raise an exception
            # self.start_auth() # Or raise AuthenticationError("Token invalid and refresh failed")
            raise Exception("Token invalid and refresh failed. Please re-authenticate.")


        if not self.token_data or 'access_token' not in self.token_data:
             raise Exception("No valid access token found. Please authenticate.")
